...
public class voiceserverThread {
	...
	public void closeThread_Server() {
	    ...
	    try {
	        if (server_socket_send != null && !server_socket_send.isClosed()) {
	    ...
	    try {
	        if (server_socket_rc != null && !server_socket_rc.isClosed()) {
	        	server_socket_rc.close();
	        }
	    } catch (Exception e) {
	        e.printStackTrace(); // hoặc ghi log nếu cần thiết
	    }
	        ...
	        try {
	            while (flag_another_speaker) {
	                ...
	                for (DatagramPacket value : Singleton.getInstance().getUsers()) {
	                	System.out.println("gửi được mà:"+ value.getAddress());
