...
public class voiceClientThread {
	...
	public void closeThread_Client() {
	    try {
	    	flag_speaker_another_client=false;
	    ...
	    try {
	        if (client_socket_send_ano != null && !client_socket_send_ano.isClosed()) {
	        	client_socket_send_ano.close();
	        }
	    } catch (Exception e) {
	        e.printStackTrace(); // hoặc ghi log nếu cần thiết
	    }
	    try {
	        if (client_socket_rc != null && !client_socket_rc.isClosed()) {
	        	client_socket_rc.close();
	        }
	    } catch (Exception e) {
	        e.printStackTrace(); // hoặc ghi log nếu cần thiết
	    }
	...
	Thread client_voice_receive = new Thread(() -> {
	    try {
	        ...
	        while (flag_speaker_another_client) {
	        	  System.out.println("chưa nhận dược"+flag_speaker_another_client);
